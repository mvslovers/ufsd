...
/* Build stamp (Issue #51).  Generated by mbt at every build: MBT_VERSION
** from the project version, MBT_COMMIT from `git rev-parse --short HEAD`
** (with "-dirty" appended when a TRACKED file differs from HEAD, and
** "unknown" outside a git checkout), MBT_COMMIT_DIRTY as the matching 0/1
** flag.  A header rather than a -DCOMMIT cflag on purpose: -MMD makes it a
** prerequisite of this object alone, so a new commit restamps the banner
** instead of leaving it advertising the commit before it (mbt#59). */
...
** lower case: MBT_VERSION and MBT_COMMIT carry the project version and a hex
...
main(int argc, char **argv)
{
    ...
    ** buffers are scoped: they are dead the moment the banner is out. */
    upcase(vers, sizeof(vers), MBT_VERSION);
    {
        char commit[24];
        char stamp[48];

        wtof("UFSD000I UFSD %s (%s) STARTING",
             vers, upcase(commit, sizeof(commit), MBT_COMMIT));
